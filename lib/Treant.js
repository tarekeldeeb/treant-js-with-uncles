    ...
    Tree.prototype = {
        ...
        setConnectionToParent: function( treeNode, hidePoint ) {
            ...
            if ( this.connectionStore[treeNode.id] ) {
                ...
                if(treeNode.uncles.length > 0 ){
                    console.log("Adding uncles for: "+ treeNode.text.name);
                    for(u in treeNode.uncles){
                        console.log('Connection Store= '+ this.connectionStore[treeNode.id].length);
                        //console.log('Connection Store= '+ JSON.stringify(this.connectionStore[treeNode.id][u]));
                        var uPathString = this.getPathString(this.getNodeDb().get(treeNode.uncles[u]._json_id), treeNode, false);
                        var uConnLine = this._R.path( uPathString );
                        this.connectionStore[treeNode.id].push(uConnLine);
                    }
                }
            }
