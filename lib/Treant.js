...
;( function() {
    ...
    Tree.prototype = {
        ...
        addUncles: function (){
            this.getNodeDb().db.forEach(function(n){
                if(!n.hidden && n.uncles.length > 0 ){
                    if ( this.tree.connectionStore[n.id].length == 1 ) {
                        console.log("Adding uncles for: "+ n.text.name);
                        const tree = this.tree;
                        for(u in n.uncles){
                            var uPathString = tree.getPathString(tree.getNodeDb().getNodeByName(n.uncles[u].text.name), n, n.stackParentId);
                            var uConnLine = tree._R.path( uPathString );
                            tree.connectionStore[n.id].push(uConnLine)
                        }
                    } else { 
                        console.log("Revealing uncles for: "+ n.text.name);
                        n.uncles.forEach(function(u, uid){
                            var uNode = this.tree.getNodeDb().getNodeByName(u.text.name)
                            if(uNode.hidden || !uNode.collapsed){
                                console.log(" --> connection to: "+u.text.name);
                                var uPathString = tree.getPathString(uNode, n, n.stackParentId);
                                var connLine = tree._R.path( uPathString );
                                this.tree.connectionStore[n.id][uid+1] = connLine;
                            } else {
                                this.tree.connectionStore[n.id].splice(uid+1,1); //remove it
                            }       
                        });
                    }
                    //console.log("Node Conn Store has: " + this.tree.connectionStore[n.id].length);
                }                             
                //connector = uConnLine; 
            })
        },
